...
restore_backup() {
    ...
    if [[ -f "$temp_restore_dir/custom_dirs_metadata.txt" ]]; then
        ...
        while IFS='|' read -r -u 3 archive_name original_path; do
             ...
             if [[ "$confirm_cust" =~ ^[yY]$ ]]; then
                  ...
                  if [[ -f "$archive_path" ]]; then
                      ...
                      if mkdir -p "$(dirname "$original_path")"; then
                          ...
                          print_message "ERROR" "Failed to create parent directory for $original_path"
                      fi
                  else
        ...
        done 3< "$temp_restore_dir/custom_dirs_metadata.txt"
    fi
